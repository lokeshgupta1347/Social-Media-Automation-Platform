...
const getOrCreateZernioProfile=async(user:any):Promise<string>=>{
    try {
        ...
        if(profiles.length>0){
            ...
            await User.findByIdAndUpdate(user._id,{zenrioProfileId:pid})
            return pid;
        ...
        if(!pid){
            throw new Error("Failed to create Zoho profile - no ID returned")
        }
        ...
        console.log("getOrCreatedZenrioProfile Error:",error?.message || error)
        throw error;
...
export const generateAuthUrl=async (req:AuthRequest,res:Response):Promise<void>=>{
    ...
    if(!authUrl){
        throw new Error(`Zoho returned no authUrl. Full response: ${JSON.stringify(data)}`)
    }
...
export const syncAccounts = async (req: AuthRequest, res: Response) : Promise<void>=> {
    try {
        ...
        const data = result.data as any;
        const zohoAccounts: any[] = data?.accounts || (Array.isArray(data) ? data : []);
