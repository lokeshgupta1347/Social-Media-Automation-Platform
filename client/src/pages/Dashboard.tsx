import { ActivityIcon, CheckCircleIcon, ClockIcon, SendIcon, Share2Icon, TrendingUpDown, TrendingUpDownIcon } from "lucide-react"
    ...
    return (
        <div className="space-y-8">
            ...
            <div>
                ...
                <p className="text-sm text-slate-400 mt-0.5">Here's what's happening with your social accounts today.</p>
            </div>
            ...
            <div className='bg-white rounded-2xl border border-slate-200 overflow-hidden'>
                <div className='flex items-center justify-between px-6 py-4 border-b border-slate-100'>
                    ...
                    <span className='text-xm text-slate-400'>{activities.length} events</span>
                </div>

                {activities.length===0 ? (
                    <div className="flex flex-col items-center justify-center py-16 px-6">
                        <div className='size-12 bg-slate-100 rounded-xl flex items-center justify-center mb-3'>
                            <ActivityIcon className="size-6 text-slate-900"/>
                            </div>
        ...
        {activities.map((activity) => (
          <div key={activity._id} className="flex items-start gap-4 px-6 py-4 hover:bg-slate-50/50 transition-colors">
            ...
            <div className="flex items-center justify-between gap-2 mb-1">
                ...
                <span className="text-xs text-shadow-slate-400 shrink-0">{new Date(activity.createdAt).toLocaleString()}</span>
