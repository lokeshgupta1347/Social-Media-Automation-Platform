            ...
            <img src="/logo.svg" alt="logo" className='size-6' />Scheduler
                ...
                <nav className='flex-1 px-3 space-y-1'>
                    {navItems.map((item)=>{
                        ...
                        return (
                            <NavLink key={item.name} to={item.path}
                                ...
                                className={`flex items-center gap-3 px-3 py-2.5 rounded-text-sm transition-all duration-200 ease-in-out ${isActive ? "bg-red-50 text-red-600 border-red-100" : "text-slate-500 hover:bg-slate-50 border-transparent hover:text-slate-700"}`}
                                >
                    ...
                    <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 transition-colors">
                        <div className="size-9 rounded-full bg-linear-to-br from-red-400 to-pink-400 flex items-center justify-center text-white font-medium shrink-0">
                    ...
                    <button onClick={logout} className="mt-1 flex items-center gap-2 px-3 py-2 w-full rounded text-sm transition-all duration-150 text-slate-500 hover:bg-red-50 hover:text-red-500">
